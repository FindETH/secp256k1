 ...
 * @return {Point}
 */
export const pointAdd = (curve: Curve, left: Point, right: Point): Point => {
  if (isEqual(left, right)) {
    return pointDouble(curve, left);
  }
 ...
 * @return {Point}
 */
export const pointDouble = (curve: Curve, point: Point): Point => {
 ...
 * @return {Point}
 */
export const pointMultiply = (curve: Curve, point: Point, value: Buffer | bigint): Point => {
  ...
  return value
    ...
    .reduce<[Point, Point | undefined]>(
      ([point, multipliedPoint], character) => {
        const doubledPoint = pointDouble(curve, point);

        if (character === '1') {
          return [doubledPoint, (multipliedPoint && pointAdd(curve, point, multipliedPoint)) || point];
        }
