 ...
 *
 * @param {Curve} curve
 * @param {Buffer | bigint} xBuffer
 * @param {boolean} isOdd
 * @return {Point}
 */
export const getPointFromX = (curve: Curve, xBuffer: Buffer | bigint, isOdd: boolean): Point => {
  const x = typeof xBuffer === 'bigint' ? xBuffer : bufferToBigInt(xBuffer);
